...
init_fs_get_part1
	...
	sta sd_sect,x
	sta fs_bootsect,x			; This is also the actual boot sector (not MBR)
	dex
...
fs_util_find_entry
	...
	bcs	fs_util_not_found		; If C then no more entries
	ldx #0						; Check name matches
fs_util_check_name
	lda fh_handle,x
	cmp fh_dir,x
	bne fs_util_find_entry
	cmp #0						; If no more bytes in name to check
	beq fs_util_found
	inx
	bra fs_util_check_name
fs_util_found
	clc							; C=0 file found
fs_util_not_found				; If C already set then not found
	ply
	plx
	pla
	rts
...
fs_open_read
	...
	jsr fs_util_find			; Try to find the file
...
fs_chdir
	...
	lda fh_handle				; First byte of name == 0?
	beq fs_chdir_root			; Then go to root
	...
	lda fh_handle+FH_FirstClust
	ora fh_handle+FH_FirstClust+1
	beq fs_chdir_root

	; Else calculate sector from this directory cluster
	ldx fh_handle+FH_FirstClust
	lda fh_handle+FH_FirstClust+1
...
fs_chdir_root					; Back to root directory
	jsr fs_rootdir
